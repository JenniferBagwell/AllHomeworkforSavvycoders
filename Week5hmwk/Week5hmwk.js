...
const nums = userData.map(userData => {
  return userData.phone;
});
console.log(nums);
const userWebEmail = userData.map(userData => [
  userData.website,
  userData.email
]);
console.log(userWebEmail);
...
const customerNine = placeHolderToDo.filter(
  placeHolderToDo => placeHolderToDo.userID == 9
);
console.log(customerNine);

const incompleteTask = placeHolderToDo.filter(
  placeHolderToDo => placeHolderToDo.completed == false
);
...
const userPost = placeHolderPost.filter(
  placeHolderPost => placeHolderPost.userID === 5
);

console.log(userPost);

const titleBody = userPost.map(userPost => [userPost.title, userPost.body]);
